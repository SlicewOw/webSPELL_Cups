...
if($loggedin && ($team_id > 0) && ($user_id < 1) && isinteam($userID, $team_id, 'admin') && empty($playerAction)) {
	...
	if($canLeftTeam) {
		...
		if($query) {
			...
			$text = '<div class="alert alert-success" role="alert">'.$_language->module['delete_ok'].'</div>';
...
} elseif($loggedin && ($team_id > 0) && isinteam($userID, $team_id, 'player') && ($user_id < 1) && !empty($playerAction)) {
