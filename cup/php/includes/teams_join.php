...
try {
    ...
    if (!$loggedin) {
        throw new \Exception($_language->module['not_loggedin'] . '<br />&raquo; <a href="index.php?site=login">Login</a>');
    }
    ...
    if(mysqli_num_rows($info) != 1) {
        throw new \Exception($_language->module['add_error6']);
    }
    ...
    if($userID == $ds['userID']) {
        // User ist Admin
        throw new \Exception($_language->module['team_stillmember']);
    } 
    
    if($teamPassword != $ds['password']) {
        // Falsches Passwort
        throw new \Exception($_language->module['wrong_pw']);
    } 
    
    if($ismember > 0) {
        // User ist bereits Mitglied
        throw new \Exception($_language->module['add_error5']);
    } 
