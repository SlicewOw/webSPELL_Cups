...
if (($cup_id > 0) && !empty($cupArray)) {
	...
	while ($db = mysqli_fetch_array($matchQuery)) {
		...
		// Match Infos
		$matchID = $db['matchID'];
		$matchArray = getmatch($matchID);

		$url = ($matchArray['active'] == 1) ?
			'index.php?site=cup&amp;action=match&amp;id='.$cup_id.'&amp;mID='.$matchID :
			'javascript:void(0);';
		...
		if ($cupArray['size'] == 64) {
			...
			if($i < 3) {
				$break = '<div class="cup_bracket_'.$cupArray['size'].'_break"></div>';
				$prefix_id = 4;
			}
		}
		...
		for ($x = 1; $x < 3; $x++) {
			...
			$teamValue = 'team'.$x;
			$spanClass = ($x == 1) ? 'cup_bracket_match_team_home' : 'cup_bracket_match_team_oppo right';

			if ($matchArray[$teamValue.'_id'] > 0) {

				if ($cupArray['mode'] == '1on1') {

					$teamName = getnickname($matchArray[$teamValue.'_id']);

				} else {

					$team_array = getteam($matchArray[$teamValue.'_id']);
