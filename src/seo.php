...
function parseWebspellURL($parameters = null)
{
    ...
    if (isset($parameters['site'])) {
        switch ($parameters['site']) {
            ...
            case 'admins':
                $returned_title[] = array($_language->module['admins']);
                break;
            ...
            case 'cup':
                ...
                if ($cup_id > 0) {
                    ...
                    $cup_name = $cupArray['name'];
                    if (!empty($action)) {
                        $cup_name .= ' - ' . ucfirst($action);
                    }
                    
                    $returned_title[] = array($cup_name);

                } else if (isset($_language->module['cup_' . $action])) {
                    $returned_title[] = array($_language->module['cup_' . $action]);    
                } else {
                    $returned_title[] = array($_language->module['cup']);    
                }

                break;
                
            case 'gameaccount':
                $returned_title[] = array($_language->module['gameaccount']);
                break;
                
            case 'hof':
                $returned_title[] = array($_language->module['hof']);
                break;
                
            case 'support':
                $returned_title[] = array($_language->module['support']);
                break;
            ...
            case 'teams':
                ...
                if ($team_id > 0) {
                    ...
                    $returned_title[] = array($_language->module['teams'] . ' - ' . $teamArray['name']);
                } else {
                    $returned_title[] = array($_language->module['teams']);    
                }
