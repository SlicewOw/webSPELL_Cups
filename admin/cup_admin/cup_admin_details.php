...
try {
	...
	if ($getAction == 'status') {
		...
		if (($getPage == 'home') && validate_array($_POST, true)) {
			...
			$navi_home 		= (empty($getPage) || (($getPage == 'home'))) ? 'btn-info white darkshadow' : 'btn-default';
			$navi_settings 	= ($getPage == 'settings') ? 'btn-info white darkshadow' : 'btn-default';
			$navi_teams 	= ($getPage == 'teams') ? 'btn-info white darkshadow' : 'btn-default';
			$navi_groups 	= ($getPage == 'groups') ? 'btn-info white darkshadow' : 'btn-default';
			$navi_bracket 	= ($getPage == 'bracket') ? 'btn-info white darkshadow' : 'btn-default';
			$navi_rules 	= ($getPage == 'rules') ? 'btn-info white darkshadow' : 'btn-default';
			$navi_act 		= ($getPage == 'activity') ? 'btn-info white darkshadow' : 'btn-default';
			...
			if ($cupArray['status'] == 1) { 
				...
				$start_url = ($cupArray['groupstage'] == 1) ? '&amp;action=start&amp;status=groupstage' : '&amp;action=start&amp;status=playoffs';
